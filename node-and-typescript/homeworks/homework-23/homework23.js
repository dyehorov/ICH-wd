...
*/
function transformToUpperCaseWithDelay(value) {
    return new Promise(resolve => {
        setTimeout(() => {
...
async function processStringsInParallel(values) {
    console.log("Task 2: parallel string processing");
    const processedValues = await Promise.all(values.map(value => transformToUpperCaseWithDelay(value)));
...
*/
function createSuccessfulPromise(name, delay) {
    return new Promise(resolve => {
        setTimeout(() => {
...
function createFailedPromise(name, delay) {
...
async function runParallelPromisesWithError() {
...
async function processNumbersWithDynamicDelay(values) {
    console.log("\nTask 4: dynamic delays");
